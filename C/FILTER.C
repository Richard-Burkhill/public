...
void fir_filter_array(float *in, float *out, unsigned int in_len, FILTER *fir)
{
    ...
    int scale)
{
    unsigned int i,j,coef_len2,acc_length;
    long acc;
    short int *in_ptr,*data_ptr,*coef_start,*coef_ptr,*in_end;
...
/* form output accumulation */
    ...
    output = (float)(output + input * (*coef_ptr)); /* input tap */
    *hist1_ptr = (float)input;                      /* last history */
...
/* allocate history array if different size than last call */
    ...
    output = (float)(input * (*coef_ptr++)); /* overall input scale factor */
